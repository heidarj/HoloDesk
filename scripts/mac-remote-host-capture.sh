...
if [[ $# -lt 1 ]]; then
  echo "usage: scripts/mac-remote-host-capture.sh <build|test|smoke> [branch] [-- <remote-script-args...>]" >&2
  exit 2
fi
...
case "$action" in
  build) script_name="host-capture-build.ps1" ;;
  test) script_name="host-capture-test.ps1" ;;
  smoke) script_name="host-capture-smoke.ps1" ;;
  *)
