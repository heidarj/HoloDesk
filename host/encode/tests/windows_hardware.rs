...
use holobridge_capture::{
    ...
    CapturedFrame, DxgiCaptureBackend,
};
...
fn hardware_encoder_can_be_created_for_primary_display() {
    ...
    let first_frame = wait_for_first_frame(&mut capture, Duration::from_secs(2));
...
fn short_capture_run_produces_at_least_one_access_unit() {
    ...
    let mut encoded_frames = encoder.encode(&first_frame).unwrap().len() as u32;
